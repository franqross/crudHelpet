...
router.post('/publicaciones_usuario', (req, res) => {
    ...
    let sql = `SELECT * FROM publicacion inner join usuario on usuario.id_usuario='${idUsuario}' WHERE usuario.id_usuario=publicacion.id_usuario AND DATE(fecha_creacion) = DATE(NOW())`
    db.query(sql, (err, rows, fields) => {
        if (err) throw err;
        else {
            res.json(rows);
